...
2. Инжектировать в конструктор типизированный `Props<YourModelProps>` первым аргументом — провайдер при резолве и при смене props вызывает на нём `set(restProps)`, в модели доступен типобезопасный `this.props.current`. Обновление привязано к текущему instance модели (корректно работает при React Strict Mode и смене props без remount).
